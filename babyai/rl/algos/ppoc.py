...
class PPOAlgo(BaseAlgo):
    ...
    def update_parameters(self):
        ...
        '''
        # TODO: add option to exps
        # TODO: add beta advantage to exps
        # TODO: add prev_options to exps
        # TODO: add inits to exps
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    option = model_results['option']
                    prev_option = model_results['prev_option']
                    ...
                    ratio = torch.exp(dist.log_prob(sb.option) - sb.log_prob)
                    surr1 = ratio * sb.advantage
                    surr2 = torch.clamp(ratio, 1.0 - self.clip_eps, 1.0 + self.clip_eps) * sb.advantage
                    inter_pi_loss = -torch.min(surr1, surr2).mean()

                    # ratio = torch.exp(dist.log_prob(sb.option) - sb.log_prob)
                    # surr1 = ratio * sb.advantage
                    # surr2 = torch.clamp(ratio, 1.0 - self.clip_eps, 1.0 + self.clip_eps) * sb.advantage
                    # beta_loss = -torch.min(surr1, surr2).mean()
                    ...
                    #
                    # q_loss = (prediction['q_o'].gather(1, sampled_options) - sampled_returns.detach()).pow(2).mul(
                    #     0.5).mean()
                    #
                    ...
                    # loss = policy_loss + beta_loss + value_loss + inter_pi_loss
                    loss = policy_loss + value_loss + inter_pi_loss
